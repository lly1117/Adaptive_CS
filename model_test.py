...
# https://github.com/sacmehta/ESPNetv2/blob/master/segmentation/cnn/cnn_utils.py
class CBR(nn.Module):
    '''
...
class D(nn.Module):
    def __init__(self, img_nf):
        ...
        self.model = mobilenet_v3(pretrained=True)
...
class Adaptive_CS(nn.Module):
    ...
    def forward(self, x, q, modes, h_sr, l_sr, gamma=0.2822):
        b, c, h, w = x.shape
        ...
        Q_unfold_backup = torch.where(Q_unfold_backup>thre, (fea_val*h_sr), (fea_val*l_sr))
        avg_cs = (Q_unfold_backup.sum() / Q_unfold_backup.numel()) / fea_val
        ...
        for i in range(self.phase_num):  # recover step-by-step
            x = self.RS[i](x, PhiT_Phi, PhiT_y, modes[i], shape_info)
